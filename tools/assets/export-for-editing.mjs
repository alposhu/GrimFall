...
const lines = [
  ...
  '  THE MOBS ROUND-TRIP. Redraw one of the nine in 02-enemies/mobs, copy it',
  '  over the matching file in art-source/mobs/, and run:',
  '',
  '      python tools/assets/build-mobs.py',
  '',
  '  That reads the palette off your drawing and rewrites src/art/mobs.js. You',
  '  are not held to 16x16 or to the old colours, and you do not have to keep',
  '  the creature symmetric: the importer crops to whatever you drew and the',
  '  renderer sizes it from the hitbox, so a wider bat is a wider bat and not a',
  '  bigger one. Two rules only — no semi-transparent pixels, and no more than',
  '  25 colours; both are checked, and both fail loudly rather than quietly',
  '  mangling the sprite.',
  '',
  '  Everything else still has to be hand-translated. A sprite in the game is a',
  '  pixel map in src/art/*.js — an array of strings plus a palette — so a',
  '  redrawn PNG has to be turned back into one of those. Ask and the same',
  '  treatment can be given to any of them.',
  '',
